        ...
        : renderer_(renderer)
{
    ...
    if (drone_surface == NULL) {
        ...
        throw std::runtime_error("Unable to load image"); // example of exception
    }

    SDL_Texture* texture = SDL_CreateTextureFromSurface(renderer_.get(), drone_surface);
    if (!texture) {
        SDL_Log("Unable to create texture: %s", SDL_GetError());
        SDL_FreeSurface(drone_surface); // Free the surface before throwing
        throw std::runtime_error("Unable to create texture"); // example of exception
    }
